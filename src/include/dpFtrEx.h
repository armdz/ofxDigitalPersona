...
# define FX_IMPORT                    __attribute__((visibility("default")))
...
FX_getVersionInfo(
  OUT FT_VERSION_INFO_PT pFxModuleVersion);  /* Pointer to the buffer containing the fingerprint feature extraction 
...
FX_getFeaturesLen(
  ...
  OUT int* FeatureSetSizeRecommended, /* Pointer to the memory receiving the size of the buffer for the fingerprint 
                                         feature set recommended for best recognition accuracy, or NULL. If NULL 
                                         is passed, minimumFeatureSetSize must not be NULL. */
  OUT int* FeatureSetSizeMinimum);    /* Pointer to the memory receiving the minimum size of the buffer for the 
...
FX_extractFeatures(
  ...
  IN  const FT_IMAGE_PTC pImage,           /* Pointer to the buffer containing the fingerprint image obtained from 
  ...
  OUT FT_IMG_QUALITY_PT pImageQuality,     /* Pointer to the buffer containing information about the quality of the 
  ...
  OUT FT_FTR_QUALITY_PT pFeaturesQuality,  /* Pointer to the buffer containing information about the quality of the 
                                              ...
                                              quality is undetermined. */
  OUT FT_BOOL* pFeatureSetCreated);        /* Pointer to the memory receiving the value of whether the fingerprint 
