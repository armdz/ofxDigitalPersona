...
#pragma once
#ifndef __FT_DP_MATCH_H
#define __FT_DP_MATCH_H
...
# define MC_IMPORT                    __attribute__((visibility("default")))
...
MC_getVersionInfo(
    OUT FT_VERSION_INFO_PT pMcModuleVersion);  /* Pointer to the structure of the software version of the
...
MC_DLL_INTERFACE FT_RETCODE
MC_getSettings(
    OUT MC_SETTINGS_PT pMcSettings); /* Pointer to the structure of the fingerprint comparison module settings */
...
MC_getSecurityLevel(
    IN FT_HANDLE mcContext, /* Handle to the comparison context */
    OUT FT_FA_RATE* pTargetFar); /* Pointer to the memory receiving the target FAR for the comparison context */
...
MC_getFeaturesLen(
    ...
    OUT int* TemplateSizeRecommended,  /* Pointer to the memory that will receive the size of the buffer for the
    ...
    OUT int* TemplateSizeMinimum);     /* Pointer to the memory receiving the minimum size of the buffer for the
...
MC_generateRegFeatures(
    ...
    IN const FT_BYTE *const FeatureSet[],/* Array of pointers to the locations of the buffers for each fingerprint
                                            feature set */
    IN int TemplateSize,                 /* Size of the fingerprint template */
    OUT FT_BYTE* pTemplate,              /* Pointer to the location of the buffer receiving the fingerprint template */
    OUT FT_BYTE Reserved1[],             /* This parameter is deprecated and should be set to NULL. */
    OUT FT_BOOL* pTemplateCreated);      /* Pointer to the memory receiving the value of whether the template is created.
...
MC_verifyFeaturesEx(
    IN FT_HANDLE mcContext,           /* Handle to the comparison context */
    IN int TemplateSize,              /* Size of the fingerprint template */
    IN const FT_BYTE* Template,       /* Pointer to the location of the buffer containing the fingerprint template */
    IN int FeatureSetSize,            /* Size of the fingerprint feature set */
    IN const FT_BYTE* FeatureSet,     /* Pointer to the location of the buffer containing the fingerprint feature set */
    ...
    OUT double* pAchievedFar,         /* Pointer to the value of the achieved FAR for this comparison. If the
                                         achieved FAR is not required, a NULL pointer can be passed. */
    OUT FT_BOOL* pComparisonDecision);/* Pointer to the memory receiving the comparison decision. This parameter
